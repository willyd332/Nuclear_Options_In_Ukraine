...
model_1_storage <- list()
model_1_c_storage <- list()
model_2_storage <- list()
model_2_c_storage <- list()
model_3_storage <- list()
model_3_c_storage <- list()
model_4_storage <- list()
model_4_c_storage <- list()

for(i in names(data)[6:10]){
  # Model 1 (General Treatment)
  model_1_storage[[i]] <- lm_robust(get(i) ~ Z, 
                             data)
  model_1_c_storage[[i]] <- lm_robust(get(i) ~ Z 
                               + warfare 
                               + russia, 
                               data)
  # Model 2 (T1 vs Control)
  model_2_storage[[i]] <- lm_robust(get(i) ~ treatment, 
                             data=subset(data, treatment!="T2"))
  model_2_c_storage[[i]] <- lm_robust(get(i) ~ treatment
                               + warfare 
                               + russia, 
                               data=subset(data, treatment!="T2"))
  # Model 3 (T2 vs Control)
  model_3_storage[[i]] <- lm_robust(get(i) ~ treatment, 
                             data=subset(data, treatment!="T1"))
  model_3_c_storage[[i]] <- lm_robust(get(i) ~ treatment
                               + warfare 
                               + russia, 
                               data=subset(data, treatment!="T1"))
  # Model 3 (T1 vs T2)
  model_4_storage[[i]] <- lm_robust(get(i) ~ treatment, 
                             data=subset(data, treatment!="T0"))
  model_4_c_storage[[i]] <- lm_robust(get(i) ~ treatment
                               + warfare 
                               + russia, 
                               data=subset(data, treatment!="T0"))
}
...
file_names = c("./regression_outputs/direct_warfare.tex",
               "./regression_outputs/indirect_aid.tex",
               "./regression_outputs/economic_sanctions.tex",
               "./regression_outputs/political_condemnation.tex",
               "./regression_outputs/general_response.tex")

captions = c("DIRECT WARFARE",
             "INDIRECT AID",
             "ECONOMIC SANCTIONS",
             "POLITICAL CONDEMNATION",
             "GENERAL DEGREE OF RESPONSE")

output_names = c("direct", "indirect", "economic", "political", "general")

for (j in 1:(length(model_1_storage))){
  texreg(list(model_1_storage[output_names[j]], model_1_c_storage[output_names[j]], 
              model_2_storage[output_names[j]], model_2_c_storage[output_names[j]], 
              model_3_storage[output_names[j]], model_3_c_storage[output_names[j]], 
              model_4_storage[output_names[j]], model_4_c_storage[output_names[j]]),
       custom.coef.names = c(
         "Intercept",
         "Z",
         "+ Opinions On Warfare",
